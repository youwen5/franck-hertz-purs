...
emission timeStep filamentVoltage randomState =
  let 
    ...
    makeElectron _ state = 
      let { value: initialEnergy, newState } = randomDouble state
      in { 
    ...
    createElectrons n state =
      ...
      in { electrons: [electron] <> electrons, newState: newState2 }
  in 
...
step state randomState =
  let 
    ...
    newVoltageSteps = 
      if state.timeStep `mod` 10 == 0 
      then state.voltageSteps <> [state.settings.acceleratingVoltage]
      else state.voltageSteps
      
    newReadings = 
      if state.timeStep `mod` 10 == 0 
      then state.readings <> [currentReading]
      else state.readings
